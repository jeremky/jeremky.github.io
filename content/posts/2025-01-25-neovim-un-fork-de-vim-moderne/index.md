...
Le principal argument mis en avant par la communauté, c'est la compatibilité avec le langage [Lua](https://fr.wikipedia.org/wiki/Lua) pour le développement de scripts.
Mais ce que je vais vous présenter dans la suite de cet article va se contenter d'utiliser des plugins développés à l'origine pour Vim. Alors pourquoi utiliser Neovim plutôt que Vim ?
